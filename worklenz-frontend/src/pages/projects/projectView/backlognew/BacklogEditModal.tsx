...
const BacklogEditModal: React.FC<Props> = ({ open, task, onClose, onSave, mode = 'edit' }) => {
  ...
  function save() {
    ...
    const current = form
    if (!current) return
    // In add mode, taskId will be generated by parent; in edit mode, it should exist
    if (mode === 'edit' && !current.taskId) return
    // taskName is required in both modes
    if (!current.taskName || !current.taskName.trim()) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative bg-white rounded shadow-lg max-w-3xl w-full mx-4 p-6">
        <h3 className="text-lg font-semibold mb-4">
          {mode === 'add' ? 'Add Task' : `Edit Task ${form.taskId}`}
        </h3>
